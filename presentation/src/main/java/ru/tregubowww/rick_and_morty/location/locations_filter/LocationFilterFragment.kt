...
import ru.tregubowww.rick_and_morty.databinding.FragmentLocationsFilterBinding
...
class LocationFilterFragment : Fragment(R.layout.fragment_locations_filter) {


    private val filterQueryLocation: FilterQueryLocation by ReadOnlyProperty { thisRef, _ ->
        requireNotNull(thisRef.requireArguments().getParcelable(FILTER_LOCATION_KEY))
    }

    private val viewBinding by viewBinding(FragmentLocationsFilterBinding::bind)

    @Inject
    lateinit var viewModelFactory: LocationFilterViewModelFactory.Factory
    private val viewModel: LocationFilterViewModel by viewModels {
        viewModelFactory.create(filterQueryLocation)
    }
    ...
    private fun locationListObserver() {
        addRepeatingJob(Lifecycle.State.STARTED) {

            viewModel.locationsFlow.collect(viewModel.locationsAdapter::submitData)
        }

        viewModel.mutableFilterLocationsFromDb
    ...
    companion object {
        fun newInstance(filterQueryLocation: FilterQueryLocation) = LocationFilterFragment().apply {
            arguments = Bundle().apply { putParcelable(FILTER_LOCATION_KEY, filterQueryLocation) }
        }

        private const val FILTER_LOCATION_KEY = "FILTER_LOCATION_KEY"
    }
}
